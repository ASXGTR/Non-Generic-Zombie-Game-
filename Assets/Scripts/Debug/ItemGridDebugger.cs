...
public class ItemGridDebugger : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        {
            ClearGrid();
            Debug.Log($"{logTag} Cleared existing slots under '{(slotParent != null ? slotParent.name : "null")}' on Start.");
        }
    }
    ...
    public void ClearGrid()
    {
        if (slotParent == null)
        {
            Debug.LogWarning($"{logTag} Slot parent not assigned.");
            return;
        }
        ...
        {
            DestroyImmediate(child.gameObject); // Immediate destruction for editor use
            removed++;
        }

        Debug.Log($"{logTag} Removed {removed} slots from '{slotParent.name}'.");
    }
    ...
    public void LogSlotChildren()
    {
        ...
        Debug.Log($"{logTag} SlotParent '{slotParent.name}' has {slotParent.childCount} children:");
